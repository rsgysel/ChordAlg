...
void EliminationOrdering::RefillBody( Vertex u, Vertex w )
{
...
void EliminationOrdering::Refill( int lower, int upper )
{
    ...
    {
        Vertex v = alpha_[ i ];
        ForEachFillPair( v, &EliminationOrdering::RefillBody );
    }
    return;
}
...
void EliminationOrdering::UnfillBody( Vertex u, Vertex w )
{
    fill_edge_count_[ u ][ w ]--;
    fill_edge_count_[ w ][ u ]--;
    if( fill_edge_count_[ u ][ w ] == 0 )
...
void EliminationOrdering::Unfill( int lower, int upper )
{
    ...
    {
        Vertex v = alpha_[ i ];
        ForEachFillPair( v, &EliminationOrdering::UnfillBody );
    }
    return;
}
